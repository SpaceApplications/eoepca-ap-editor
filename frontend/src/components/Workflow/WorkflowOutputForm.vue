...
export default {
  ...
  methods: {
    ...
    handleOutputSourceChange(newValue){
      const trimmedValue = newValue.replace(/\s/g, '')
      if (trimmedValue.split(',').filter(e => e).length > 1) {
        this.$set(this.output, 'outputSource', trimmedValue.split(','));
      } else {
        this.$set(this.output, 'outputSource', trimmedValue);
      }
    },
  ...
  computed: {
    ...
    outputSourceValue() {
      if (this.output.type && this.output.outputSource?.length) return {id: this.output.outputSource[0], type: this.output.type};
      return undefined;
    },
    availableOutputs() {
