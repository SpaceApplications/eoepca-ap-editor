...
FILE_SYSTEM_BASE_PATH = os.getenv(
    "AP_EDITOR_FILES_DIRECTORY", os.path.join(os.getcwd(), "files/")
)
...
async def list_application_package_versions(ap_slug: str) -> List[ApplicationVersion]:
    ...
    if os.path.isdir(ap_path):
        versions = [f.path for f in os.scandir(ap_path)]  # Splice to remove BASE_PATH
        print(versions)
        version_list = []
        for v in versions:
            locked = False
        ...
        return version_list
    raise HTTPException(
        status_code=404, detail="Application package with given slug not found"
    )


@app.get("/aps/{ap_slug}/versions/{version_slug}/")
async def get_application_package_version(
    ap_slug: str, version_slug: str
) -> ApplicationCWL:
...
async def create_or_update_application_package_version(
    ap_slug: str, version_slug: str, cwl: CWL
):
    ap_path = os.path.join(FILE_SYSTEM_BASE_PATH, ap_slug)
    ...
    for symbol in ILLEGAL_SYMBOLS:
        if symbol in ap_slug or symbol in version_slug:
            raise HTTPException(
                status_code=400, detail=f"Application package slug or version slug cannot contain special symbols."
            )
...
async def delete_application_package(ap_slug: str) -> None:
    ap_path = os.path.join(FILE_SYSTEM_BASE_PATH, ap_slug)
    if not os.path.isdir(ap_path):
        raise HTTPException(status_code=404, detail="Application slug not found.")
    versions = [
